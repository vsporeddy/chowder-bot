...
async def play_timed(bot, ctx, players):
    """
    play is the main gameplay loop.
    ...
    await display_timed(ctx, message_to_guess, choices)
    guesses = await get_choice_timed(bot, ctx, choices, players)
    winners = []
    blacklist = []
    for player, guess in guesses.items():
        if guess != answer:
            blacklist.append(bot.get_user(player))
        elif guess == answer:
            winners.append(bot.get_user(player))
            break
...
async def display_timed(ctx, msg, choices):
    embed = discord.Embed(
        title="Who said this?",
        #description=f'"{msg.content}"'
        description = msg
    )
    choice_list = [f"{number}. {user}" for number, user in choices.items()]
    embed.add_field(name="Choices", inline=False, value='\n'.join(choice_list))
    embed.add_field(name="Timer", inline=False, value=printProgressBar(0, WHOSAIDITTIMER, prefix = 'Progress:', suffix = 'Complete', length = 50, fill = '\|'))
    ...
    for index, field in enumerate(embed.fields):
            if field.name == 'Timer':
                timer_index = index
                break
    for i in range(0, WHOSAIDITTIMER, WHOSAIDITTICK):
        await asyncio.sleep(WHOSAIDITTICK)
        embed.remove_field(timer_index)
        embed.insert_field_at(index, name="Timer", inline=False, value=printProgressBar(i + 1, WHOSAIDITTIMER, prefix = 'Progress:', suffix = 'Complete', length = 50, fill = '\|'))
...
def printProgressBar (iteration, total, prefix = '', suffix = '', decimals = 1, length = 100, fill = '█'):
    ...
    ret = (f'\r{prefix} |{bar}| {percent}% {suffix}')
    return ret

async def get_choice_timed(bot, ctx, choices, players, message):
    new_msg = await ctx.channel.fetch_message(message.id)
    guesses = defaultdict(list)
    for index, reaction in enumerate(new_msg.reactions):
        if reaction.emoji in emojis:
            async for user in reaction.users():
                guesses[guess.author.id].append(index)
