...
class Governance(commands.Cog):
    ...
    async def nomination_helper(self, ctx, is_promotion):
        ...
        name = chowder.get_name(nominator)
        ...
        if (is_promotion and nominee.top_role.position + 1 >= config["promotion_cap"]) or \
           (not is_promotion and nominee.top_role.position >= config["promotion_cap"]):
            await ctx.send(
                f"Sorry {name}, no democratic promotions/demotions at **{nominee.top_role.name}** "
        ...
        if noms_needed > 0:
            await ctx.send(
                f"Hey {chowder.get_collective_name()}, {nominator.mention} has nominated "
            ...
            if is_promotion:
                await ctx.send(
                    f"{config['promotion_emote']} Congratulations {nominee.mention}, you just got promoted "
                    f"from **{current_rank.name}** to **{new_rank.name}**!"
                )
            else:
                await ctx.send(
                    f"{config['demotion_emote']} Yikes {nominee.mention}, by popular demand you've been "
